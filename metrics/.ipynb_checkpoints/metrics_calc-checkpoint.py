...
class MetricsEngine:
    ...
    def _calculate_volume_ratio_30d(self, data: pd.DataFrame) -> float:
        ...
        if len(data) > 1:
            ...
            if len(time_diffs) > 0:
                interval_td = time_diffs.mode()[0]
                interval_minutes = int(interval_td.total_seconds() / 60)
        
        # Calculate candles per 24 hours
        candles_per_24h = int(1440 / interval_minutes)
        
        # Need at least 24h worth of candles
        if len(data) < candles_per_24h:
            self.logger.info(f"Not enough data for 24h volume (need {candles_per_24h} candles, have {len(data)})")
            return np.nan
        
        # Calculate 24h volume
        volume_24h = data['volume'].iloc[-candles_per_24h:].sum()
        
        # For 30-day average, calculate candles per day and total for 30 days
        candles_per_day = int(1440 / interval_minutes)
        candles_30_days = candles_per_day * 30
        
        if len(data) >= candles_30_days:
            # Get last 30 days
            last_30_days = data.iloc[-candles_30_days:]
            ...
            complete_days = len(data) // candles_per_day

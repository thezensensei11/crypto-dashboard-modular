...
class PriceMetrics:
    ...
    def calculate_price_metrics(price_data: pd.DataFrame, symbol: str) -> Dict[str, float]:
        ...
        try:
            ...
            if len(price_data) > 1:
                time_diffs = price_data['timestamp'].diff().dropna()
                # Get most common time difference
                interval_td = time_diffs.mode()[0] if len(time_diffs) > 0 else pd.Timedelta(minutes=60)
                interval_minutes = int(interval_td.total_seconds() / 60)
                logger.debug(f"{symbol}: Detected interval of {interval_minutes} minutes")
            else:
                interval_minutes = 60  # Default to hourly
                logger.warning(f"{symbol}: Cannot detect interval, assuming hourly")
            
            # Calculate candles per 24 hours
            candles_per_24h = int(1440 / interval_minutes)  # 1440 minutes in 24 hours
            
            # Current price (most recent close)
            if len(price_data) > 0:
                metrics['Price'] = price_data['close'].iloc[-1]
            
            # 24h return and volume change (based on detected interval)
            if len(price_data) >= candles_per_24h:
                # Price 24h ago
                price_24h_ago = price_data['close'].iloc[-candles_per_24h]
                price_return_24h = ((metrics['Price'] - price_24h_ago) / price_24h_ago) * 100
                metrics['24h Return %'] = price_return_24h
                
                # 24 hour volume (sum of last N candles based on interval)
                volume_24h = price_data['volume'].iloc[-candles_per_24h:].sum()
                
                # Volume change calculation
                # Compare last 24 hours volume to previous 24 hours
                if len(price_data) >= (2 * candles_per_24h):
                    # Previous 24 hours total volume
                    previous_volume = price_data['volume'].iloc[-(2*candles_per_24h):-candles_per_24h].sum()
                    ...
                    logger.info(f"{symbol}: Not enough data for 24h volume comparison (need {2*candles_per_24h} candles, have {len(price_data)})")
                
                # New metric: 24h volume / 30-day average daily volume
                # Calculate candles per day and total candles for 30 days
                candles_per_day = int(1440 / interval_minutes)
                candles_30_days = candles_per_day * 30
                
                if len(price_data) >= candles_30_days:
                    # Get last 30 days of data
                    last_30_days = price_data.iloc[-candles_30_days:]
                    ...
                    available_days = len(price_data) // candles_per_day
                    if available_days >= 2:  # Need at least 2 days for meaningful average
                        ...
                        logger.info(f"{symbol}: Not enough data for volume ratio (need at least {2 * candles_per_day} candles, have {len(price_data)})")
            else:
                logger.info(f"{symbol}: Not enough data for 24h metrics (only {len(price_data)} candles, need {candles_per_24h})")

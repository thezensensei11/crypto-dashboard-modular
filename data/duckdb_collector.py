...
class BinanceDataCollector:
    ...
    def _safe_tz_convert(self, timestamp, target_tz=None):
        """Safely convert timestamp to target timezone or naive"""
        try:
            if target_tz is None:
                # Convert to naive
                if hasattr(timestamp, 'tz'):
                    return timestamp.tz_convert(None) if timestamp.tz else timestamp
                else:
                    return pd.Timestamp(timestamp).tz_localize(None)
            else:
                # Convert to specific timezone
                if hasattr(timestamp, 'tz'):
                    return timestamp.tz_convert(target_tz) if timestamp.tz else timestamp.tz_localize(target_tz)
                else:
                    return pd.Timestamp(timestamp).tz_localize(target_tz)
        except:
            ...
            if not force_cache and self.db_manager:
                try:
                    ...
                    if not cached_data.empty:
                        ...
                        # Fix timezone comparison once and for all
                        try:
                            # Convert both to naive datetime for comparison
                            if hasattr(latest, 'tz'):
                                latest_naive = latest.tz_convert(None) if latest.tz else latest
                            else:
                                latest_naive = pd.Timestamp(latest).tz_localize(None)
                            
                            if hasattr(end_date, 'tz'):
                                end_naive = end_date.tz_convert(None) if end_date.tz else end_date
                            else:
                                end_naive = pd.Timestamp(end_date).tz_localize(None)
                            
                            # If we have recent data (within 2 hours), use cache
                            if latest_naive > end_naive - timedelta(hours=2):
                            ...
                            # If timezone comparison fails, just fetch new data
                            self.logger.debug(f"Timezone comparison failed, fetching new data: {e}") self._ensure_min_data(cached_data, symbol, interval, lookback_days)
                except Exception as e:
    ...
    def _ensure_min_data(
        ...
        if hasattr(data_min_timestamp, 'tz') and data_min_timestamp.tz is not None:
            ...
            compare_timestamp = pd.Timestamp(data_min_timestamp, tz="UTC")
